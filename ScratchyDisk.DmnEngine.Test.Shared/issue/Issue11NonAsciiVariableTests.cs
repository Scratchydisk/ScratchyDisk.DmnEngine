...
using ScratchyDisk.DmnEngine.Engine.Definition.Builder;
using ScratchyDisk.DmnEngine.Engine.Execution;
using ScratchyDisk.DmnEngine.Engine.Execution.Context;
using ScratchyDisk.DmnEngine.Parser.Dto;
using Variable = ScratchyDisk.DmnEngine.Engine.Definition.Builder.Variable;

namespace ScratchyDisk.DmnEngine.Test.Issue
{
    [TestClass]
    [TestCategory("Issue Fix Tests")]
    public class Issue11NonAsciiVariableTests
    {
        [TestMethod]
        [DataRow("Stræøåščřáů", typeof(string))]
        [DataRow("Booleanæøåščřáů", typeof(bool))]
        [DataRow("Integeræøåščřáů", typeof(int))]
        [DataRow("Doubleæøåščřáů", typeof(double))]
        [DataRow("Objæøåščřáů", typeof(object))]
        ...
        public void DecisionRefAndRequireTest()
        {
            ...
            var def = new DmnDefinitionBuilder()
                .WithInput("input1æøåščřáů", out var input1Ref)
                .WithInput("input2æøåščřáů", out var input2Ref)
                .WithInput("input3æøåščřáů", out var input3Ref)
                .WithVariable<string>("outputæøåščřáů", out var outputVarRef)
                .WithExpressionDecision("decision1", ed => ed
                        .Put("input1æøåščřáů").To(outputVarRef)
                        .Requires(input1Ref),
                    out var decision1Ref)
                .WithExpressionDecision("decision2", "outputæøåščřáů+2", outputVarRef, out var decision2Ref) //IMPORTANT: The "output" variable is string, so the "decision2" is not numeric addition but it's string concat
                .WithExpressionDecision("decision3", ed => ed
                        .Put("input2æøåščřáů+outputæøåščřáů").To(outputVarRef)
                        .Requires(input2Ref)
            ...
            ri1[0].Name.Should().Be("input1æøåščřáů");
            ...
            ri3[0].Name.Should().Be("input2æøåščřáů");
            ...
            ri5[0].Name.Should().Be("input1æøåščřáů");
            ri5[0].IsInputParameter.Should().BeTrue();
            ri5[1].Name.Should().Be("input3æøåščřáů");
            ...
            ri7[0].Name.Should().Be("input1æøåščřáů");
            ...
            ctx
                .WithInputParameter("input1æøåščřáů", 4)
                .WithInputParameter("input2æøåščřáů", 8);
            ...
            ctx.Snapshots.Last["outputæøåščřáů"].Value.Should().Be("8222");
        }
        ...
        {
            var builder = new DmnDefinitionBuilder()
                .WithInput("input1æøåščřáů")
                .WithInput<int>("input2æøåščřáů", out var input2Ref)
                .WithVariable<bool>("var1æøåščřáů", out var var1Ref)
                .WithVariable<int>("var2æøåščřáů", out var var2Ref)
                .WithExpressionDecision("decision1", "input1æøåščřáů>5.1", var1Ref)
                .WithExpressionDecision("decision2", "input2æøåščřáů+2", var2Ref, out var decision2Ref)
                .WithExpressionDecision("decision3", ed => ed
                    .Put("input2æøåščřáů % 2")
                    .To(var2Ref))
                .WithExpressionDecision("decision4", ed => ed
                    .Put("input1æøåščřáů>var2æøåščřáů")
                    .To(var1Ref)
                    .Requires(input2Ref)
                    .Requires(decision2Ref));
            ...
            {
                { "input1æøåščřáů", input1 },
                { "input2æøåščřáů", input2 }
            };
            ...
            if (!isExecErr)
            {
                ...
                execResult2.First["var2æøåščřáů"].Value.Should().Be(result2);
                execResult3.First["var2æøåščřáů"].Value.Should().Be(result3);
        ...
        [DataRow(6.1, 6, false, true, 8, 0, false)]
        [DataRow("6", 6, false, false, 8, 0, false)] //input1 is string, FEEL treats "6">5.1 as incomparable → false
        public void NormalizedExpressionDecisionTest(object input1, int input2, bool isExecErr, bool result1, int result2, int result3, bool result4)
        {
            var builder = new DmnDefinitionBuilder()
                .WithInput("  input_1 æøåščřáů  ")
                .WithInput<int>("   input 2 æøåščřáů", out var input2Ref)
                .WithVariable<bool>("var_1æøåščřáů", out var var1Ref)
                .WithVariable<int>("  var_2æøåšč 99_88   řáů", out var var2Ref)
                .WithExpressionDecision("decision1", "input_1_æøåščřáů>5.1", var1Ref)
                .WithExpressionDecision("decision2", "input_2_æøåščřáů+2", var2Ref, out var decision2Ref)
                .WithExpressionDecision("decision3", ed => ed
                    .Put("input_2_æøåščřáů % 2")
                    .To(var2Ref))
                .WithExpressionDecision("decision4", ed => ed
                    .Put("input_1_æøåščřáů>var_2æøåšč_99_88___řáů")
                    .To(var1Ref)
                    .Requires(input2Ref)
                    .Requires(decision2Ref));
            ...
            if (!isExecErr)
            {
                ...
                execResult1.First["var_1æøåščřáů"].Value.Should().Be(result1);
                execResult2.First["var_2æøåšč_99_88___řáů"].Value.Should().Be(result2);
                execResult3.First["var_2æøåšč_99_88___řáů"].Value.Should().Be(result3);
                execResult4.First["var_1æøåščřáů"].Value.Should().Be(result4);
            }
            else
            {
                Action act = () => ctx.ExecuteDecision("decision1");
        ...
        public void ExpressionDecisionAdvancedTest()
        {
            ...
            var builder = new DmnDefinitionBuilder()
                .WithInput<int>("input1æøåščřáů")
                .WithInput<int>("input2æøåščřáů")
                .WithVariable<int>("var0æøåščřáů", out var var0Ref);
            var variables = new Dictionary<string, Variable.Ref>();
            for (var i = 1; i <= 3; i++)
            {
                builder.WithVariable<int>($"var{i}æøåščřáů", out var varRef);
            ...
            result0.First["var0æøåščřáů"].Value.Should().Be(input1 * 3);
            var result1 = ctx.ExecuteDecision("decision1");
            result1.First["var1æøåščřáů"].Value.Should().Be(input2 + 1);
            var result2 = ctx.ExecuteDecision("decision2");
            result2.First["var2æøåščřáů"].Value.Should().Be(input2 * 2);
            var result3 = ctx.ExecuteDecision("decision3");
            result3.First["var3æøåščřáů"].Value.Should().Be(input2 + 3);


            ExpressionDecision ExpressionDecisionBuilder(ExpressionDecision.ExpressionDecisionSrcBuilder b)
            {
                return b.Put("input1æøåščřáů*3").To(var0Ref);
            ...
            {
                return b.Put($"input2æøåščřáů+{i}").To(outVar);
            }
        ...
        public void TableTest(int input1, string input2, bool input3, bool isErr, int? resultOut1, string resultOut2, string match)
        {
            ...
            var def = new DmnDefinitionBuilder()
                .WithInput<int>("input 1 æøåščřáů", out var input1Ref)
                .WithInput<string>("input2æøåščřáů", out var input2Ref)
                .WithInput<bool>("input3æøåščřáů")
                .WithVariable<bool>("var 1 æøåščřáů", out var var1Ref)
                .WithVariable<int>("var 2 æøåščřáů", out var var2Ref)
                .WithVariable<string>("var3æøåščřáů", out var var3Ref)
                .WithExpressionDecision("ed", "(input_1_æøåščřáů % 2)==0", var1Ref, out var edRef)
                .WithTableDecision("tbl", table =>
                    table
                        ...
                        .WithInput("(input_1_æøåščřáů % 4)", out var tblInput4Ref, "0", "1", "2")
                        .WithInput("!input3æøåščřáů", out var tblInput5Ref)
                        ...
                        .WithRule("r3", "input1 (even && %4 = 0)-> input1+2,N/A", r => r
                            ...
                            .And(tblInput4Ref, "0")
                            .Then(tblOutput1Ref, "input_1_æøåščřáů+2"))
            ...
            def.Variables["input_1_æøåščřáů"].IsInputParameter.Should().BeTrue();
            def.Variables["input_1_æøåščřáů"].Type.Should().Be<int>();
            def.Variables["input2æøåščřáů"].IsInputParameter.Should().BeTrue();
            def.Variables["input2æøåščřáů"].Type.Should().Be<string>();
            def.Variables["input3æøåščřáů"].IsInputParameter.Should().BeTrue();
            def.Variables["input3æøåščřáů"].Type.Should().Be<bool>();

            def.Variables["var_1_æøåščřáů"].IsInputParameter.Should().BeFalse();
            def.Variables["var_1_æøåščřáů"].Type.Should().Be<bool>();
            def.Variables["var_2_æøåščřáů"].IsInputParameter.Should().BeFalse();
            def.Variables["var_2_æøåščřáů"].Type.Should().Be<int>();
            def.Variables["var3æøåščřáů"].IsInputParameter.Should().BeFalse();
            def.Variables["var3æøåščřáů"].Type.Should().Be<string>();
            ...
            tbl.Inputs[0].Variable.Name.Should().Be("input_1_æøåščřáů");
            tbl.Inputs[0].Expression.Should().BeNull();
            tbl.Inputs[0].AllowedValues.Should().BeNull();
            tbl.Inputs[1].Variable.Name.Should().Be("var_1_æøåščřáů");
            tbl.Inputs[1].Expression.Should().BeNull();
            tbl.Inputs[1].AllowedValues.Should().BeNull();
            tbl.Inputs[2].Variable.Name.Should().Be("input2æøåščřáů");
            ...
            tbl.Inputs[3].Expression.Should().Be("(input_1_æøåščřáů % 4)");
            ...
            tbl.Inputs[4].Expression.Should().Be("!input3æøåščřáů");
            tbl.Inputs[4].Variable.Should().BeNull();
            tbl.Inputs[4].AllowedValues.Should().BeNull();

            tbl.Outputs.Should().HaveCount(2);
            tbl.Outputs[0].Variable.Name.Should().Be("var_2_æøåščřáů");
            tbl.Outputs[0].AllowedValues.Should().BeNull();
            tbl.Outputs[1].Variable.Name.Should().Be("var3æøåščřáů");
        ...
        public void TableTestCollectSum(int input1, int input2, int output, string match)
        {
            ...
            // "r4" - always -> 1000
            var def = new DmnDefinitionBuilder()
               .WithInput<int>("input1æøåščřáů", out var inputVar1Ref)
               .WithInput<int>("input2æøåščřáů", out var inputVar2Ref)
               .WithVariable<int>("outputæøåščřáů", out var outputVarRef)
               .WithTableDecision("tbl", table =>
                   table
                       ...
                       .WithRule("r1", "input1 > 10 -> input1", r => r
                           ...
                           .Then(tblOutputRef, "input1æøåščřáů"))

                       .WithRule("r2", "input1 >=5 && <15, input2 is 1|3|input1 -> input2/2", r => r
                           .When(tblInput1Ref, "[5..15[")
                           .And(tblInput2Ref, "1,3,input1æøåščřáů")
                           .Then(tblOutputRef, "input2æøåščřáů/2"))

                       .WithRule("r3", "input2 !5 && !input1 -> 10", r => r
                           .When(tblInput2Ref, "not(5,input1æøåščřáů)")
            ...
            var ctx = DmnExecutionContextFactory
                .CreateExecutionContext(def)
                .WithInputParameter("input1æøåščřáů", input1)
                .WithInputParameter("input2æøåščřáů", input2);

            var result = ctx.ExecuteDecision("tbl");
            result.HasResult.Should().BeTrue();
            result.IsSingleResult.Should().BeTrue();
            result.First["outputæøåščřáů"].Value.Should().Be(output);
        ...
        public void TableTestCollectList(int input1, int input2, string match)
        {
            ...
            tbl.Rules[1].Inputs[1].UnparsedExpression.Should().Be("1,3,input1æøåščřáů");
            tbl.Rules[2].Inputs[0].UnparsedExpression.Should().Be("not(5,input1æøåščřáů)");
            ...
            for (var i = 0; i < hits.Length; i++)
            {
                ...
                if (ruleName == "r1") singleResult["outputæøåščřáů"].Value.Should().Be(input1);
                if (ruleName == "r2") singleResult["outputæøåščřáů"].Value.Should().Be(input2 / 2);
                if (ruleName == "r3") singleResult["outputæøåščřáů"].Value.Should().Be(10);
                if (ruleName == "r4") singleResult["outputæøåščřáů"].Value.Should().Be(1000);
            }
        ...
        public void SfeelInputExpressionHelperTest()
        {
            var dummy = new DmnDefinitionBuilder()
                .WithInput("inputæøåščřáů", out var iRef)
                .WithVariable<int>("varæøåščřáů", out var vRef);
            ...
            s3.Should().Be("inputæøåščřáů");
            var s4 = SFeel.Eq(false);
            s4.Should().Be("false");
            var s5 = SFeel.Eq(vRef, 1, "A");
            s5.Should().Be("varæøåščřáů,1,\"A\"");
        ...
        {
            var def = new DmnDefinitionBuilder()
                .WithInput<int>("inputæøåščřáů", out var inputRef)
                .WithVariable<int>("outputæøåščřáů", out var outputRef)
                .WithTableDecision("tbl", table =>
                    table
            ...
            def.Variables["inputæøåščřáů"].IsInputParameter.Should().BeTrue();
            def.Variables["inputæøåščřáů"].Type.Should().Be<int>();
            def.Variables["outputæøåščřáů"].IsInputParameter.Should().BeFalse();
            def.Variables["outputæøåščřáů"].Type.Should().Be<int>();
            def.Decisions.Should().HaveCount(1);
            def.Decisions["tbl"].Should().BeOfType<DmnDecisionTable>();

            var tbl = (DmnDecisionTable)def.Decisions["tbl"];
            tbl.Inputs.Should().HaveCount(1);
            tbl.Inputs[0].Variable.Name.Should().Be("inputæøåščřáů");
            tbl.Outputs[0].Variable.Name.Should().Be("outputæøåščřáů");
            ...
            {
                ctx.WithInputParameter("inputæøåščřáů", input);
            }
        ...
        public void TableOutputsTest(int input, int? outputInt, bool? outputBool, string outputStr, int varInt, bool varBool, string varStr)
        {
            var def = new DmnDefinitionBuilder()
                .WithInput<int>("inputæøåščřáů", out var inputRef)
                .WithVariable<int>("output1æøåščřáů", out var output1Ref)
                .WithVariable<bool>("output2æøåščřáů", out var output2Ref)
                .WithVariable<string>("output3æøåščřáů", out var output3Ref)
                ...
                .WithTableDecision("tbl", table =>
                    table
                        ...
                        .WithRule("rule2", r => r.When(tblInputRef, "[2..3]")
                            .Then(tblOutput1Ref, "inputæøåščřáů+1")
            ...
            def.Variables["inputæøåščřáů"].IsInputParameter.Should().BeTrue();
            def.Variables["inputæøåščřáů"].Type.Should().Be<int>();
            def.Variables["output1æøåščřáů"].IsInputParameter.Should().BeFalse();
            def.Variables["output1æøåščřáů"].Type.Should().Be<int>();
            def.Variables["output2æøåščřáů"].IsInputParameter.Should().BeFalse();
            def.Variables["output2æøåščřáů"].Type.Should().Be<bool>();
            def.Variables["output3æøåščřáů"].IsInputParameter.Should().BeFalse();
            def.Variables["output3æøåščřáů"].Type.Should().Be<string>();
            ...
            tbl.Outputs[0].Variable.Name.Should().Be("output1æøåščřáů");
            tbl.Outputs[1].Variable.Name.Should().Be("output2æøåščřáů");
            tbl.Outputs[2].Variable.Name.Should().Be("output3æøåščřáů");
            ...
            result.IsSingleResult.Should().BeTrue();
            if (outputInt != null) result.First["output1æøåščřáů"].Value.Should().Be(outputInt);
            if (outputBool != null) result.First["output2æøåščřáů"].Value.Should().Be(outputBool);
            if (outputStr != null) result.First["output3æøåščřáů"].Value.Should().Be(outputStr);
            snap.Should().NotBeNull();
            snap["output1æøåščřáů"].Value.Should().Be(varInt);
            snap["output2æøåščřáů"].Value.Should().Be(varBool);
            snap["output3æøåščřáů"].Value.Should().Be(varStr);
        }
        ...
        public void VariableErr()
        {
            ...
            Action act;
            var def = new DmnDefinitionBuilder()
                .WithVariable<string>("æøåščřáů_æøåščřáů");

            //valid non-ascii letters
            act = () => def.WithVariable<string>("æøåščřáů");
            act.Should().NotThrow();
            act = () => def.WithVariable<string>("æ1ø2å3š4č5ř6á7ů");
            act.Should().NotThrow();
